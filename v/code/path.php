...
class path{ //Save the give contents to a file
    ...
    public function __construct(string $iname, bool $is_file,bool $add_root) {
        //
        //use the current working directory (cwd) to resolve relative path
        $name = schema::resolve_path($iname);
        //
        //Set the full name, adding the root if necessary
        $this->name = $add_root ? $_SERVER['DOCUMENT_ROOT'].$name: $name;
        //
