...
export class io extends view.view {
    ...
    options) {
        ...
        const result = io.get_io_type(anchor, type, options);
        //
    ...
    static get_io_type(anchor, type, options) {
        //
        ...
        if (options?.col !== undefined)
            return io.deduce_io(anchor, options?.col);
        //
    ...
    col) {
        ...
        if (col.length > 100)
            return new textarea(anchor, undefined, { col });
        //
...
export class select extends io {
    ...
    options) {
        ...
        if (!col)
            throw new schema.mutall_error(`No choices found for this selector`);
        //
...
export class file extends input {
    ...
    // 
    constructor(anchor, type, options) {
        // 
...
export class textarea extends input {
    ...
    textarea;
    //
    constructor(anchor, size, options) {
        //
...
class choices extends io {
    type;
    //
    constructor(anchor, type = 'multiple', options) {
        super(anchor, 'primary', options);
        this.type = type;
    }
